...
@Scene(SCENES.ADD_SHOPS)
export class AddShopsSceneService {
	constructor(
	...
	async selectSomeShops(@Context() ctx: MyContext) {
		...
		await this.userServiceRepository.updateObservedById(userId, keyIds, true);
		const user = await this.userServiceRepository.findOneById(userId);

		if (user) {
			const {initHour, timezone} = user;
			const text = this.utilsService.createTimeText(successAddShop, initHour, timezone);
			await this.menuService.createShopsText(ctx, text);
		}
		return ctx.scene.leave();
	}
	...
	async selectAllShops(@Context() ctx: MyContext) {
		const userId = ctx.session.telegramUserId;
		await this.userServiceRepository.updateObserved(userId, true);
		const user = await this.userServiceRepository.findOneById(userId);
	...
	@Action(ACTIONS.SELECT_ALL_SHOPS)
	async getCheckingShops(@Context() ctx: MyContext) {
		const userId = ctx.session.telegramUserId;
